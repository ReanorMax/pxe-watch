...
def _run_playbook_async(tags, started):
    """Execute ansible-playbook in background and stream logs to journald."""
    cmd = ["ansible-playbook", ANSIBLE_PLAYBOOK, "-i", ANSIBLE_INVENTORY]
    if tags:
        cmd.extend(["--tags", ",".join(tags)])

    # Start the process and stream stdout/stderr in real time so that
    # /logs/ansible shows progress while the playbook is running.
    ...
    def _stream(pipe, log_fn, collector, filter_fn=lambda _: True):
    ...
    stderr_thread = threading.Thread(
        ...
        args=(
            ...
            logging.error,
            stderr_lines,
            lambda l: not warning_re.match(l),
        ),
    )
    stdout_thread.start()
    stderr_thread.start()
    ...
    non_warning_lines = [
        line for line in stderr_lines if not warning_re.match(line)
    ]
